...
uint64_t str2flag(const char *str) {
    ...
    if (!strcasecmp("ip_compress", str)) flag |= FLAG_IP_COMPRESS;
    if (!strcasecmp("conv_compress", str)) flag |= FLAG_CONV_COMPRESS;
...
#define CASE(_f, _l) \
    ...
    CASE(FLAG_IP_COMPRESS, conv_compress);
    CASE(FLAG_CONV_COMPRESS, conv_compress);
#undef CASE
