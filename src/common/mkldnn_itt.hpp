...
/**
 * @def MKLDNN_ITT_DOMAIN(domainName)
 * @ingroup ie_dev_profiling
 ...
 */
#define MKLDNN_ITT_DOMAIN(domainName)                                                               \
inline mkldnn::impl::domain_t domainName() noexcept                                                 \
{                                                                                                   \
    static auto d = mkldnn::impl::internal::domain(#domainName);                                    \
    return d;                                                                                       \
}

/**
 * @def MKLDNN_ITT_SCOPED_TASK(domain, handleOrTaskName)
 * @ingroup ie_dev_profiling
 ...
 */
#define MKLDNN_ITT_SCOPED_TASK(domain, taskOrTaskName)                                              \
        struct Task ## __LINE__ {};                                                                 \
        mkldnn::impl::ScopedTask<domain> ittScopedTask ## __LINE__                                  \
                    (mkldnn::impl::handle<Task ## __LINE__>(taskOrTaskName));
