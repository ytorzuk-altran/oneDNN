...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Vmm>::compute_ker(int ur_w, int pad_l,
    ...
    auto input_offset = [=](int oi, int ic, int ki) {
        return jcp.typesize_in
                * ((ki * (jcp.dilate_w + 1) + oi * stride_w - pad_l)
                          * jcp.ic_without_padding * jcp.ngroups + ic_sub_step * ic);
    };
    ...
    for (int ki = 0; ki < kw; ki++) {
        ...
        int tail_size = jcp.ic_without_padding % ic_sub_step;
...
void _jit_avx512_core_x8s8s32x_fwd_kernel<Vmm>::generate()
{
    ...
    // todo:
    int in_ic_shift = jcp.ic_without_padding * jcp.ngroups;
...
//    }
    ...
    if (!jcp.is_1stconv && !(jcp.ic % jcp.ic_block == 0)) return status::unimplemented;
