...
template <cpu_isa_t isa, typename Wmm>
void jit_uni_eltwise_injector_f32<isa, Wmm>::compute_cmp_mask(const Vmm &vmm_src,
...
void jit_uni_eltwise_injector_f32<isa, Wmm>::mish_compute_vector_fwd(
    ...
    exp_compute_vector_fwd(vmm_src);
    h->uni_vmovups(vmm_aux0, vmm_src);
    h->uni_vsubps(vmm_aux0, vmm_aux0, table_val(minus_two));
    h->uni_vmulps(vmm_src, vmm_src, vmm_aux0);
...
template <cpu_isa_t isa, typename Wmm>
void jit_uni_eltwise_injector_f32<isa, Wmm>::hswish_compute_vector_fwd(
        const Vmm &vmm_src) {
    // Save src data on stack for later usage
    h->sub(h->rsp, vlen);
    h->uni_vmovups(h->ptr[h->rsp], vmm_src);
...
template struct jit_uni_eltwise_injector_f32<avx512_common, Xbyak::Xmm>;
