...
jit_avx512_core_ymm_1x1_conv_kernel::jit_avx512_core_ymm_1x1_conv_kernel(
        const jit_1x1_conv_conf_t &ajcp, const primitive_attr_t &attr,
        const memory_desc_t &dst_md)
    : jcp(ajcp), attr_(attr) {}

void jit_avx512_core_ymm_1x1_conv_kernel::bcast_loop(int load_loop_blk) {
...
Address jit_avx512_core_ymm_1x1_conv_kernel::output_ptr(
        const bool is_out_layout_nxc, const int i_load, const int i_ur) {
    if (one_of(jcp.prop_kind, forward_training, forward_inference,
                backward_data)) {
        int i_load_shift = is_out_layout_nxc ? jcp.load_block
                                             : jcp.bcast_dim * jcp.load_block;
...
static int vreg_accum_idx(
        const int load_loop_blk, const int ur, const int i_load, const int i_ur) {
    return (i_ur + i_load * ur);
}
...
void jit_avx512_core_ymm_1x1_conv_kernel::apply_postops(
    ...
    for (int i = 0; i < p.len(); i++) {
        ...
        if (post_op.is_eltwise()) {
            ...
            for (int j = 0; j < load_loop_blk; ++j) {
                int start_idx = vreg_accum_idx(load_loop_blk, ur, j, 0);
            ...
            for (int j = 0; j < load_loop_blk; ++j) {
                int s_idx = vreg_accum_idx(load_loop_blk, ur, j, 0);
...
void jit_avx512_core_ymm_1x1_conv_kernel::reduce_loop(
    ...
    auto vreg_load = [=](int i_load, int i_fma) {
        return Ymm(utils::rnd_up(ur * load_loop_blk, jcp.fma_step)
                + jcp.fma_step * i_load + i_fma);
    };

    auto vreg_accum = [=](int i_load, int i_ur) {
        return Ymm(vreg_accum_idx(load_loop_blk, ur, i_load, i_ur));
    };
    ...
    auto store = [=]() {
        ...
        test(aux_reg_output_data, cpu_isa_traits<avx2>::vlen - 1);
...
void jit_avx512_core_ymm_1x1_conv_kernel::generate() {
    const auto &p = attr_.post_ops_;
    int end_idx = jcp.with_dw_conv ? p.find(primitive_kind::convolution) : p.len();
    for (int i = 0; i < end_idx; i++) {
        auto &post_op = p.entry_[i];
        if (post_op.is_eltwise()) {
            eltwise_injectors.push_back(new jit_uni_eltwise_injector_f32<avx512_common, Ymm>(
                    this,
                    post_op.eltwise
            ));
        } else if (post_op.is_depthwise()) {
            depthwise_injectors.push_back(new jit_uni_depthwise_injector_f32<avx512_common, Ymm>(
                    this,
                    post_op.depthwise.alg
            ));
        } else if (post_op.is_quantization()) {
            quantization_injectors.push_back(new jit_uni_quantization_injector_f32<avx512_common, Ymm>(
                    this,
                    post_op,
    ...
    const int simd_w = isa_simd_width_;
...
bool jit_avx512_core_ymm_1x1_conv_kernel::post_ops_ok(
...
status_t jit_avx512_core_ymm_1x1_conv_kernel::init_conf(
    ...
    jcp.with_eltwise = post_ops.find(primitive_kind::eltwise) != -1;
    if (jcp.with_eltwise) {
    ...
    if (everyone_is(data_type::f32, src_d.data_type(), weights_d.data_type(),
                dst_d.data_type())) {
        format_tag_t wei_tag = pick(2 * (ndims - 3) + with_groups, OIw8i8o,
...
void jit_avx512_core_ymm_1x1_conv_kernel::init_scratchpad(
...
void jit_avx512_core_ymm_1x1_conv_kernel::balance(jit_1x1_conv_conf_t &jcp) {
